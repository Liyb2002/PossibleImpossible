...
class generate_helper:
    def __init__(self, generic_object_list):
        ...
        self.generate_part(foreground_type, foreground_connect, foreground_intersection, steps, True)
        self.generate_part(background_type, background_connect, background_intersection, steps, False)
        self.connect()
        
    
    def generate_part(self, start_type, connect_direction, intersection_pos, steps, isFront):
    ...
    def connect(self):
        ...
        for i in range(len(self.particle_list)):
            ...
            if self.particle_list[i].success:
                print("success")
                result_particle = self.particle_list[i]
                break
